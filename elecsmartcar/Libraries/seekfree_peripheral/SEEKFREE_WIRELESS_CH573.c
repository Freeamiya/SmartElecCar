...
static  fifo_struct     xdata wireless_ch573_fifo;
static  uint8           xdata wireless_ch573_buffer[WIRELESS_CH573_BUFFER_SIZE];  // 数据存放数组

static          uint8   xdata wireless_ch573_data;
...
//  @note       
//-------------------------------------------------------------------------------------------------------------------
void wireless_ch573_callback(uint8 dat)
{
    // 接到一个字节后单片机将会进入串口中断，通过在此处读取dat可以取走数据
    fifo_write_buffer(&wireless_ch573_fifo, &dat, 1);       // 存入 FIFO
}
...
void wireless_ch573_init(void)
{
	...
	wireless_module_uart_handler = wireless_ch573_callback;
